...
import { Send, Shield, Info, Lock, Loader2, MessageSquare, MoreVertical, X, StickyNote, User } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col h-full min-w-0">
        ...
        <div className="p-4 border-t border-zinc-100 bg-white">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className={cn(
                "flex-1 rounded-2xl px-4 py-3 border transition-all flex items-center gap-2",
                isNoteMode 
                ...
                <button 
                  onClick={handleSend}
                  disabled={!inputText.trim() || (room.status === 'PENDING' && !isNoteMode)}
                  className={cn(
                    "p-2 rounded-xl text-white transition-all disabled:opacity-50 flex-shrink-0",
                    isNoteMode ? "bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-200" : "bg-[#8B1D1D] hover:bg-[#6D1616] shadow-lg shadow-red-200"
                  )}
                >
